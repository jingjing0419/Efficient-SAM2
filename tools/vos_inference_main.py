...
@torch.inference_mode()
@torch.autocast(device_type="cuda", dtype=torch.bfloat16)
def vos_separate_inference_per_object_wj(
    predictor,
    base_video_dir,
    input_mask_dir,
    output_mask_dir,
    ...
    for object_id in object_ids:
        ...
        # run propagation throughout the video and collect the results in a dict
        for out_frame_idx, _, out_mask_logits in predictor.propagate_in_video_wj(
            inference_state,
...
def vos_separate_inference_per_object(
    ...
    """
    if False: # sim_count
        with open('/home/zhangjing/sam2_Proj/hahaha/sim_count/sim_count_davis.txt', 'a', encoding='utf-8') as f:
...
def main():
    ...
    def set_predictor():
        ...
        predictor.WB_theta = args.WB_theta
        predictor.obj_theta = args.obj_theta
        predictor.sel_max_iou = args.sel_max_iou
        ...
        if args.apply_WB:
            if args.apply_bypass:
                build_bypass_model(args, predictor, adapter_type=args.bypass_type, adapter=True, training=False)
                predictor.image_encoder.trunk.blocks[args.final_global_layer].bypass_branch.load_state_dict(torch.load(args.bypass_ckpt))
                logger.info('load bypass chackpoint from {}'.format(args.bypass_ckpt))
                predictor.image_encoder.trunk.blocks[args.final_global_layer].bypass_branch.eval()
                bypass_params = sum(p.numel() for p in predictor.image_encoder.trunk.blocks[args.final_global_layer].bypass_branch.parameters())
                logger.info('The parameter of bypass: {}'.format(bypass_params))
            else:
                build_bypass_model(args, predictor, adapter=False, training=False)
                logger.info('straight forward bypass')
        else:
            logger.info('Original model without bypass')
    ...
    parser.add_argument(
        "--apply_bypass",
        action="store_true",
        help='apply bypass network. if false, apply identity passes')
    ...
    parser.add_argument(
        '--bypass_ckpt_base',
        type=str,
        # default='./bypass/ckpt/bypass_20250814_131218_EP_9_Tr160_D5_EP10_epoch9.pth',
        default=None,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help='dynamic memory token pruning theta')
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='disable SWR method (original forward)')
    ...
    parser.add_argument(
        ...
        type=str,
        default='SAV_test',
        help="SAV_test, SAV_val, DAVIS, MOSE, MOSEv2, SeCVOS",
    )
    parser.add_argument(
        "--bypass_type",
        type=str,
        default='bottleneck',
        help="bottleneck, attention",
    )
    parser.add_argument(
        '--WB_all_layer',
        action='store_true',
        default=False,
        help="bypass in earlier layers")
    
    parser.add_argument(
        '--Mem_Frame_Prune',
        action='store_true',
        default=False,
        help="prune similar memory frames")
    ...
    if args.sam2_model=='base+':
        args.bypass_ckpt = args.bypass_ckpt_base
        ...
        if args.WB_all_layer:
            args.selected_layers = [i for i in range(0, 21)]
            args.win_sel_layer=[0,3,6]
            args.fpn_feat_layer=[1,4]
            args.scale_layer=[2,5]
        else:
        ...
        args.bypass_ckpt = args.bypass_ckpt_large
        
        if args.WB_all_layer:
            args.selected_layers = [i for i in range(0, 44)]
            args.win_sel_layer=[0,3,9]
            args.fpn_feat_layer=[1,7]
            args.scale_layer=[2,8]
        else:
    ...
    if args.apply_WB:
        if args.apply_bypass:
            SWR = 'WB-{}'.format(int(args.WB_theta*10))
        else:
            SWR = 'WBST-{}'.format(int(args.WB_theta*10))
        if args.disable_WB:
            SWR = ''
        if args.WB_all_layer:
            SWR  = SWR+'_ALL'
        output_folder_name = output_folder_name+'_'+SWR
    if args.prune_memory:
        if args.topk_mask:
            SMR = 'MeP-topk{}'.format(int(args.set_drop_ratio*100))
        elif args.random_mask:
            SMR = 'MeP-random{}'.format(int(args.set_drop_ratio*100))
        elif args.uniform_mask:
            SMR = 'MeP-uniform{}'.format(int(args.set_drop_ratio*100))
        else:
            SMR = ''
    else:
        if args.pool_memory:
            ...
            SMR = ''
    output_folder_name = output_folder_name+'_'+SMR
    ...
    for n_video, video_name in enumerate(video_names):
        ...
        if not args.track_object_appearing_later_in_video:
            if args.apply_WB:
                ...
                drop_ratio_log[video_name] = {}
                for i, layer in enumerate(predictor.memory_attention.layers):
                    drop_ratio_log[video_name][i] = layer.cross_attn_image.drop_ratio_log
                # print(video_name, drop_ratio_log[video_name])
                for i, DR_i in drop_ratio_log[video_name].items():
                    logger.info('{} - MA_layer_{} - mean DR = {}'.format(video_name, i, sum(DR_i)/(len(DR_i)+1e-6)))
                
                WS_Sel = []
                for win in predictor.WS_log['Sel']:
                    if len(win) > 0:
                        WS_Sel.append(win)
                WS_Sel_nums = [len(a) for a in WS_Sel]
                
                WS_PF = []
                for win in predictor.WS_log['PF']:
                    if len(win) > 0:
                        WS_PF.append(win)
                WS_PF_nums = [len(a) for a in WS_PF]
                
                WS_Mask = []
                for win in predictor.WS_log['Mask']:
                    if len(win) > 0:
                        WS_Mask.append(win)
                WS_Mask_nums = [len(a) for a in WS_Mask]
                
                WS_Sel_nums_avg = sum(WS_Sel_nums)/(len(WS_Sel_nums)+1e-6)
                WS_PF_nums_avg = sum(WS_PF_nums)/len(WS_PF_nums) if len(WS_PF_nums)!=0 else 0
                WS_Mask_nums_avg = sum(WS_Mask_nums)/(len(WS_Mask_nums)+1e-6)
                
                logger.info('***** Window Selection Report [{}({} frames)]  *****'.format(video_name, len(WS_Sel_nums)))
                logger.info('Average Selected Window Nums:         {:.4f}'.format(WS_Sel_nums_avg))
                logger.info('Average Perception Focus Window Nums: {:.4f}'.format(WS_PF_nums_avg))
                logger.info('Average Mask Window Nums (TP={:.2f}%): {:.4f}'.format(len(WS_Mask)/(len(WS_Sel)+1e-6)*100, WS_Mask_nums_avg))
                # print(FW_time_log[video_name]['attn'])
            else:
                vos_inference(
                    predictor=predictor,
                    base_video_dir=args.base_video_dir,
            ...
            if args.apply_WB:
                vos_separate_inference_per_object_wj(
                    predictor=predictor,
                    base_video_dir=args.base_video_dir,
                    input_mask_dir=args.input_mask_dir,
                    output_mask_dir=args.output_mask_dir,
