...
class SAM2Base(torch.nn.Module):
    def __init__(
        ...
        memory_attention,
        memory_encoder,
        num_maskmem=12,  # default 1 input frame + 6 previous frames
        image_size=512,
    ...
    def track_step(
        ...
        if self.enable_MeP_info:
            ...
            if self.mem_info['enable_mem_prune'] and (frame_idx-1) % self.memory_temporal_stride_for_eval == 0:
                ...
                # ---mem_merge_ids_per_obj = self.mem_info['mem_merge_ids_stack'].setdefault(self.mem_info['obj_idx'], [])
                if len(mem_masks_per_obj) == 5:
                    mem_masks_per_obj.pop(0)
                    # ---mem_merge_ids_per_obj.pop(0)
                mem_masks_per_obj.append(Mem_ATMs)
                # ---mem_merge_ids_per_obj.append(Mem_merge_ids)
