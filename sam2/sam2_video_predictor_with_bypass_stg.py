...
# def convert_indices(window_indices, old_feature_size=64, new_feature_size=256,
#                             old_window_size=14, new_window_size=8):
#     """优化后的坐标计算方法"""
#     device = window_indices.device
#     scale_factor = new_feature_size / old_feature_size  # 4
#     old_num_windows = old_feature_size // old_window_size  # 5
#     new_num_windows = new_feature_size // new_window_size  # 32
    
#     # 1. 一次性计算所有起始坐标
#     h_start = ((window_indices // old_num_windows) * old_window_size * scale_factor).long()
#     w_start = ((window_indices % old_num_windows) * old_window_size * scale_factor).long()
    
#     # 2. 使用向量化操作生成偏移量
#     h_offsets = torch.arange(0, old_window_size * scale_factor, new_window_size, device=device)
#     w_offsets = torch.arange(0, old_window_size * scale_factor, new_window_size, device=device)
    
#     # 3. 生成网格并计算索引
#     h_indices = (h_start.unsqueeze(-1) + h_offsets.unsqueeze(0)) // new_window_size
#     w_indices = (w_start.unsqueeze(-1) + w_offsets.unsqueeze(0)) // new_window_size
    
#     # 4. 使用广播计算所有新索引
#     new_indices = (h_indices.unsqueeze(-1) * new_num_windows + 
#                   w_indices.unsqueeze(1)).reshape(-1)
#     new_indices = new_indices.to(dtype=torch.int64)
    
#     return torch.unique(new_indices, sorted=True)

def convert_indices(window_indices, ori_feat_size=64, ori_window_size=14, num_windows=5, target_feat_size=256, target_window_size=8):
    
    # 创建一个mask tensor
    mask = torch.zeros((num_windows*num_windows), dtype=torch.bool, device=window_indices.device)
    mask[window_indices] = True  # [5,5]的mask，标记需要设置的窗口位置
    mask = mask.reshape(num_windows, num_windows)
    # 扩展mask到完整大小
    mask = mask.repeat_interleave(ori_window_size, dim=0).repeat_interleave(ori_window_size, dim=1)
    mask = mask[:ori_feat_size, :ori_feat_size]
    resized_mask = F.interpolate(mask.float().unsqueeze(0).unsqueeze(0), 
                       size=(target_feat_size, target_feat_size), 
                       mode='nearest')
    resized_mask = resized_mask.squeeze(0).squeeze(0)  # 转回bool类型
    
    target_num_windows = target_feat_size // target_window_size
    resized_mask = resized_mask.view(target_num_windows, target_window_size, target_num_windows, target_window_size)
    windows = resized_mask.permute(0,2,1,3).reshape(target_num_windows*target_num_windows, -1).sum(-1)
    indices = torch.nonzero(windows).squeeze(-1)
...
def cumulative_threshold_selection(weights, threshold=0.8):
    """
    基于累积贡献度的CUDA高效批量选择
...
class SAM2VideoPredictor_bypass_stg(sam2_bypass):
    ...
    @classmethod
    def from_pretrained(cls, model_id: str, **kwargs) -> "SAM2VideoPredictor_bypass_stg":
        """
    ...
    def select_mask_windows_v2(self, masks):
        ...
        H, W = masks.shape
        # 特征图尺寸是64*64, mask尺寸是256*256，需要统一比例
        window_size = self.mem_info['window_sizes'][-2] * 4
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                ...
                if frame_idx in obj_output_dict["cond_frame_outputs"]:
                    ...
                    if frame_idx not in []:
                    # if frame_idx not in [25,26,27,28,29,30]:
    ...
    def propagate_in_video_wj(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            if frame_idx > 1 and self.prune_memory:
                self.mem_info['enable_mem_prune'] = True
                # self.enable_mem_prune=True
            ...
            if frame_idx %1 == 0:
                if not self.disable_WB:
                    ...
                    # Pt_ATMs = F.unfold(Pt_ATMs, kernel_size=14, stride=14)
                    B = Pt_ATMs.shape[0]
                    if self._WB_info['window_size'] == 14:
                    ...
                    # PF_win_id = torch.where(Pt_ATMs > 0.04)[1]
                    # print(PF_win_id.shape)
                    # print(torch.unique(PF_win_id))
                    ...
                    win_sel_layer = self.win_sel_layer
                    scale_layer = self.scale_layer
                    if len(self.mem_info['sel_win_ids']) == 0 :
                        # self.mem_info['sel_win_ids'] = {}
                        self.mem_info['sel_win_ids'][win_sel_layer[2]] = [i for i in range(25)] if 'base' in self.sam2_model else [i for i in range(16)]
                        for layer_id in win_sel_layer[:-1]+scale_layer:
                            self.mem_info['sel_win_ids'][layer_id] = [i for i in range(1024)]
                        # self.mem_info['sel_win_ids'][win_sel_layer[0]] = self.mem_info['sel_win_ids'][win_sel_layer[1]] = [i for i in range(1024)]
                        # self.mem_info['sel_win_ids'][0] = [i for i in range(1024)]
                        print('restart full')
                        print('selected windows:    ',self.mem_info['sel_win_ids'])
                    else:
                        masks = torch.any(masks, dim=0)
                        if self.dilate_mask:
                        ...
                        if mask_win_id.shape[0] == 0 or any(x < 3 for x in self.mem_info['obj_scores']):
                            ...
                            PF_win_id = cumulative_threshold_selection(Pt_ATMs, 0.6)
                            # PF_win_id = torch.where(Pt_ATMs > 0.1)[1]
                            # print(PF_win_id)
                            # print(self.mem_info['obj_scores'])
                        else:
                            # PF_win_id = adaptive_significant_selection(Pt_ATMs, 3)
                            # PF_win_id = cumulative_threshold_selection(Pt_ATMs, 0.6)
                            PF_win_id = torch.tensor([], device=device, dtype=torch.int32)
                            # PF_win_id = torch.where(Pt_ATMs > 0.3)[1]
                            # self.mem_info['sel_win_id'] = self.select_mask_windows(masks)
                        # self.mem_info['sel_win_id'] = -1
                        if 'base' in self.sam2_model:
                            window_size = 14
                            num_windows= 5
                        else:
                            window_size = 16
                            num_windows= 4
                        self.mem_info['sel_win_ids'][win_sel_layer[2]] = torch.unique(torch.cat([PF_win_id, mask_win_id]), sorted=True)
                        High_Resolution_indices = convert_indices(self.mem_info['sel_win_ids'][win_sel_layer[2]], 
                                                                                                                                          ori_window_size=window_size, 
                                                                                                                                          num_windows=num_windows)
                        for layer_id in win_sel_layer[:-1]+scale_layer:
                            self.mem_info['sel_win_ids'][layer_id] = High_Resolution_indices
                        # self.mem_info['sel_win_ids'][win_sel_layer[1]] = self.mem_info['sel_win_ids'][win_sel_layer[0]]
                        
                        # self.mem_info['obj_scores'] = []
                        # self.mem_info['sel_win_id'] = mask_win_id
                        if self.print_WS or frame_idx % 50 ==0:
                            print('selected windows:    ',self.mem_info['sel_win_ids'][win_sel_layer[2]])
                            print('selected windows all:    ',self.mem_info['sel_win_ids'].keys(), self.mem_info['sel_win_ids'][0])
                            print('Prompt Focus windows:',torch.unique(PF_win_id))
                            # print('**** Mask windows ***',mask_win_id)
                            print(torch.unique(mask_win_id))
                        self.WS_log['Sel'].append(self.mem_info['sel_win_ids'][win_sel_layer[2]])
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                ...
                if frame_idx in obj_output_dict["cond_frame_outputs"]:
                    ...
                    if frame_idx % 3 != 0 and frame_idx != end_frame_idx:
                        ...
                        if frame_idx >0 and len(self.mem_info['sel_win_ids']) != 0:
                            ...
                            if self.accu_cnt == self.train_steps or frame_idx==end_frame_idx:
                                ...
                                self.optimizer.zero_grad()
                                torch.cuda.empty_cache()
                                avg_loss = self.accumulated_loss / self.train_steps
                                self.iters += 1
                                print(f"=================frame {frame_idx+1}, Step {self.iters}, avg Loss: {avg_loss:.4f}=================")
                                if self.use_wandb:
                                    try:
            ...
            if frame_idx %1 == 0:
                ...
                if len(self.mem_info['sel_win_ids']) == 0 :
                    ...
                    mask_win_id = self.select_mask_windows(masks)
                    if mask_win_id.shape[0] == 0 or any(x < 3 for x in self.mem_info['obj_scores']):
                        # PF_win_id = torch.where(Pt_ATMs > 0.1)[1]
                        PF_win_id = cumulative_threshold_selection(Pt_ATMs, 0.6)
                        # print(PF_win_id)
